...
Resolver.prototype.hasNew = function (canonicalDir, pkgMeta) {
    ...
    // Otherwise call _hasNew with both the package meta and the canonical dir
        ...
        metaFile = path.join(canonicalDir, '.block.json');
...
Resolver.prototype._savePkgMeta = function (meta) {
    ...
    ['template'].forEach(function (attr) {
        if (meta[attr]) return;

        that._logger.log(
            'warn', 'invalid-meta',
            (meta.name || 'component') + ' is missing "' + attr + '" entry in block.json'
        );
    });
    ...
    return Q.nfcall(fs.writeFile, path.join(this._tempDir, '.block.json'), contents)
    .then(function () {
