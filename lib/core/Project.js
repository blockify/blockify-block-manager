...
Project.prototype.saveJson = function (forceCreate) {
    ...
    if (!this._jsonFile && !forceCreate) {
        this._logger.warn('no-json', 'No block.json file to save to, use blockify init to create one');
        return Q.resolve();
    }

    file = this._jsonFile || path.join(this._config.cwd, 'block.json');
...
Project.prototype._readJson = function () {
    ...
    .spread(function (json, deprecated, assumed) {
        ...
        if (!assumed) {
            that._jsonFile = path.join(that._config.cwd, deprecated ? deprecated : 'block.json');
        }
...
Project.prototype._readInstalled = function () {
    ...
    return this._installed = Q.nfcall(glob, '*/.block.json', {
        cwd: componentsDir,
        dot: true
    })
