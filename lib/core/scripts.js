...
var orderByDependencies = function (packages, installed, json) {
    ...
    var depsFromBlockifyJson = json && json.dependencies ? mout.object.keys(json.dependencies) : [];
    var packageNames = mout.object.keys(packages);

    //get the list of the packages that are specified in block.json in that order
    //its nice to maintain that order for users
    var desiredOrder = mout.array.intersection(depsFromBlockifyJson, packageNames);
    //then add to the end any remaining packages that werent in block.json
...
var run = function (cmdString, action, logger, config) {
    ...
    //pass env + BLOCKIFY_PID so callees can identify a preinstall+postinstall from the same blockify instance
    var env = mout.object.mixIn({ 'BLOCKIFY_PID': process.pid }, process.env);
