...
function getNewVersion(currentVersion, versionArg) {
    ...
    if (!newVersion) {
        throw createError('Invalid version argument: `' + versionArg + '`. Usage: `blockify version [<newversion> | major | minor | patch]`', 'EINVALIDVERSION');
    }
...
function gitCommitAndTag(newVersion, message) {
    ...
    return Q.nfcall(execFile, 'git', ['add', 'block.json'], {env: process.env})
    .then(function () {
