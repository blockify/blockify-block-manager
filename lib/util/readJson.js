...
var blockifyJson = require('blockify-json');
var Q = require('q');

// The valid options are the same as blockify-json#readFile.
...
function readJson(file, options) {
    ...
    // Read
    return Q.nfcall(blockifyJson.read, file, options)
    .spread(function (json, jsonFile) {
        ...
        if (err.code === 'ENOENT' && options.assume) {
            return [blockifyJson.parse(options.assume, options), false, true];
        }
